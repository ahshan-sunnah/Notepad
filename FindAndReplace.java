...
public class FindAndReplace extends JDialog implements ActionListener {
    ...
    public FindAndReplace(JFrame owner, boolean isReplace) {
        ...
        setResizable(true);
        int x = (owner.getWidth() * 10 / 5) - (getWidth() / 2);
        int y = (owner.getHeight() * 10 / 5) - (getHeight() / 2);
        setLocation(x, y);
        setVisible(true);
    ...
    private void process() {
        ...
        int caret = Note_Pad.getArea().getCaretPosition();
        String word = getWord();
    ...
    private int search(String text, String word, int caret) {
        ...
        Note_Pad.getArea().setCaretPosition(0);
        if (found) {
            Note_Pad.getArea().requestFocus();
            if (isScarchDown()) {
                Note_Pad.getArea().select(caret, caret + check);
            } else {
                Note_Pad.getArea().select(caret - check, caret);
            }
            //for replace
            if (isReplace) {
                String replace = replaceText.getText();
                Note_Pad.getArea().replaceSelection(replace);
                if (isScarchDown()) {
                    System.out.println(caret);
                    System.out.println(check);
                    Note_Pad.getArea().select(caret, caret + replace.length());
                } else {
                    Note_Pad.getArea().select(caret - replace.length(), caret);
                }
            }
            foundOne = true;
            return caret;
    ...
    private String getAllText() {
        if (caseNotSelected()) {
            return Note_Pad.getArea().getText().toLowerCase();
        }
        return Note_Pad.getArea().getText();
    }
    ...
    private void replaceAll() {
        ...
        String text = Note_Pad.getArea().getText();
        ...
        Note_Pad.getArea().setText(sb.toString());
